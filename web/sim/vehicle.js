...
import { idmAcceleration, idmFreeFlow, idmStep, IDM_DEFAULTS, ROAD_SPEEDS } from './idm.js';
import { decideLaneChange } from './mobil.js';
...
export class SimVehicle {
    ...
    _advanceToNextEdge() {
        ...
        if (this.routeIdx < this.route.length) {
            ...
            // Plan a new route — use iterative approach to avoid stack overflow
            let attempts = 0;
            while (this.route.length === 0 && attempts < 3) {
                this._planNewRoute();
                attempts++;
            }
            if (this.route.length > 0) {
                // Advance to first edge without recursion
