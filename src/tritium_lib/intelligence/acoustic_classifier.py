...
def _extract_wav_features(wav_path: str) -> Optional[tuple[str, list[float], float, float, float, float, int]]:
    ...
    if not wav_path.lower().endswith(".wav"):
        logger.warning("Rejected non-WAV file: %s", wav_path)
        return None

    try:
        real_path = os.path.realpath(wav_path)
        if not os.path.isfile(real_path):
            logger.warning("WAV path is not a regular file: %s", wav_path)
            return None
    ...
    try:
        ...
        if file_size > MAX_WAV_SIZE:
            logger.warning("WAV file too large (%d bytes): %s", file_size, wav_path)
            return None
        if file_size < 44:
            logger.warning("WAV file too small (%d bytes): %s", file_size, wav_path)
            return None
    except OSError:
        return None
    ...
    try:
        with wave_mod.open(real_path, "rb") as wf:
            ...
            if n_frames > max_frames:
                logger.warning("WAV file has too many frames (%d): %s", n_frames, wav_path)
                return None
...
def train_from_wav_directory(
    ...
    if not wav_path.is_dir():
        logger.warning("WAV training directory does not exist: %s", wav_dir)
        ...
        with open(csv_path, "r") as f:
            ...
            for row in reader:
                ...
                if ".." in filename or filename.startswith("/"):
                    logger.warning("Rejected suspicious filename in CSV: %s", filename)
                    continue
    ...
    logger.info(
        "Extracted %d training samples from WAV files (%d classes)",
        len(training_data),
...
class MFCCClassifier:
    ...
    def __init__(self, k: int = 5) -> None:
        ...
        self._sklearn_knn: Optional[object] = None
        self._sklearn_scaler: Optional[object] = None
    ...
    def train(self, data: Optional[list] = None) -> None:
        ...
        if self._use_sklearn:
            try:
                ...
                logger.info(
                    "MFCC classifier trained (sklearn KNN k=%d) on %d samples, %d classes",
                    self.k, n_samples, self._training_class_count,
                )
                return
            except Exception as exc:
                logger.warning("sklearn training failed, falling back to pure-python: %s", exc)
        ...
        logger.info(
            "MFCC classifier trained (pure-python KNN k=%d) on %d samples, %d classes",
            self.k, n_samples, self._training_class_count,
        )
    ...
    def classify(self, features: AudioFeatures) -> AcousticEvent:
        if self._ml_classifier and features.mfcc:
            try:
                ...
                logger.debug("ML classification failed, falling back: %s", exc)
