...
The PTZ pose estimator accepts any object exposing the PTZPosition
protocol (pan/tilt + optional limits) so it stays decoupled from any
particular SensorNode/Position implementation.
"""
...
@runtime_checkable
class PTZPosition(Protocol):
    """Structural type for a PTZ position with optional discovered limits.

    Any object with these attributes (e.g. a SensorNode Position dataclass)
    can be passed to PoseEstimator.update() without a hard import dependency.
    """
...
class PoseEstimator:
    ...
    def update(self, position: PTZPosition) -> CameraPose:
