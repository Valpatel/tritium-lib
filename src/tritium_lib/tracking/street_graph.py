...
def _latlng_to_local(
    ...
    Uses the same math as tritium_lib.geo but without depending on the
...
class StreetGraph:
    ...
    def load_from_elements(
        self,
        elements: list[dict],
        ref_lat: float,
        ref_lng: float,
    ) -> None:
        """Build graph directly from Overpass-style element dicts.

        Useful for testing or when you already have OSM data.
        """
        self._build_graph(elements, ref_lat, ref_lng)
        if self.graph is None or len(self.graph.nodes) == 0:
            self.graph = None
    ...
    def node_count(self) -> int:
        """Number of nodes in the graph, or 0 if not loaded."""
        if self.graph is None:
            return 0
    ...
    def edge_count(self) -> int:
        """Number of edges in the graph, or 0 if not loaded."""
        if self.graph is None:
            return 0
