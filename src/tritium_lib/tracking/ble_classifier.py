...
class BLEClassifier:
    ...
    ----------
    event_bus:
        EventBus instance (duck-typed, needs ``.publish(topic, data)``).
    known_macs:
    ...
    training_store_fn:
        Optional callable that returns a training store instance.
        Used to log classification decisions for the RL pipeline.
        If None, training store logging is silently skipped.
    """
    ...
    def __init__(
        ...
        training_store_fn: Optional[Callable[[], Any]] = None,
    ) -> None:
    ...
    def _log_classification(self, classification: BLEClassification) -> None:
        ...
        if self._training_store_fn is None:
            return

        try:
            store = self._training_store_fn()
        except Exception:
            return
        if store is None:
            return
