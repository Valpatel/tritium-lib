...
if TYPE_CHECKING:
    from ..events.bus import QueueEventBus as EventBus
    from ..store.dossiers import DossierStore
    from .target_tracker import TargetTracker

logger = logging.getLogger("dossier_manager")

__all__ = ["DossierManager", "entity_type_for_class"]

# COCO detector class -> Tritium entity_type.  The operational mission tracks
# "cars, phones, people, animals" as first-class targets, so every class an
# operator can see on the map must resolve to a real entity_type instead of
# silently collapsing to "unknown" (which leaves the dossier headline reading
# "Unknown" for a clearly-classified contact).
_VEHICLE_CLASSES = frozenset({
    # COCO vehicle classes plus the generic "vehicle" label the synthetic
    # detector emits directly.
    "vehicle", "car", "motorcycle", "bicycle", "truck",
    "bus", "train", "boat", "airplane",
})
_ANIMAL_CLASSES = frozenset({
    # COCO animal classes plus the generic "animal" label.
...
_PHONE_CLASSES = frozenset({
    ...
    # COCO calls them "cell phone"; map the common synonyms onto the same
    # "phone" entity_type the BLE presence handler already emits so RF and
    # vision fuse onto one taxonomy instead of vision phones falling through
    # to "unknown".
    "cell phone", "cellphone", "phone", "smartphone", "mobile phone",
})

# Generic catch-all entity_types that must never overwrite a more specific
# prior classification.  A BLE sighting only knows "device"; the dossier may
# already hold "phone"/"wearable" from a richer observation, so when fusing a
# generic type we raise confidence but pass ``entity_type=None`` to leave the
# specific type intact (mirrors the vision path's "unknown never clobbers").
_GENERIC_ENTITY_TYPES = frozenset({"unknown", "device"})


def entity_type_for_class(class_name: str) -> str:
    """Map a detector class label to a Tritium entity_type.

    Returns ``"unknown"`` only for classes outside the
    people/vehicle/animal/phone taxonomy, so the unrecognised case is explicit
    rather than the default.
    """
    cn = (class_name or "").strip().lower()
    if cn == "person":
        return "person"
    if cn in _PHONE_CLASSES:
        return "phone"
    if cn in _VEHICLE_CLASSES:
        return "vehicle"
    if cn in _ANIMAL_CLASSES:
        return "animal"
    return "unknown"


def _specific_entity_type(entity_type: str | None) -> str | None:
    """Return ``entity_type`` only if it is specific enough to fuse.

    Generic placeholders ("unknown", "device") are downgraded to ``None`` so
    ``update_classification`` raises confidence without clobbering a stronger
...
class DossierManager:
    ...
    prune_interval:
        Seconds between retention prune runs (default 3600 = hourly).
        Set to 0 to disable.
    prune_max_signal_age_s:
        Drop ``dossier_signals`` older than this many seconds for any
        non-exempt dossier (exempt = pinned OR tagged ``'vip'``).
        ``None`` (default) defers to the store's configured TTL, which
        honors the ``TRITIUM_DOSSIER_SIGNAL_TTL_DAYS`` env var and falls
        back to 30 days.  Pass an explicit value only to override both.
    prune_max_signals_per_dossier:
        Cap signal history at this count per dossier; oldest signals are
        cycled out first (default 10000).

    Retention policy (Gap-fix C M-8)
    --------------------------------
    Behavioral SAT (W204) measured ``dossiers.db`` at 5.3 GB with
    a ~50 GB / 90-day unbounded-growth projection.  Periodic pruning
    enforces a hard ceiling:

      * **Age-based** — signals older than the age cutoff are dropped,
        **except** for exempt dossiers: ``pinned = True`` OR tagged
        ``'vip'`` (which retain their full history forever).
      * **Per-dossier cap** — each dossier keeps at most
        ``prune_max_signals_per_dossier`` signals; the oldest are
        cycled out when the cap is exceeded.  This applies even to
        pinned/VIP dossiers.

    Pin a dossier with ``store.set_pinned(dossier_id, True)`` or tag it
    ``'vip'`` to opt out of the age-based prune (e.g. for known persons
    of interest).
    """
    ...
    def __init__(
        ...
        prune_max_signal_age_s: float | None = None,
        prune_max_signals_per_dossier: int = 10000,
    ) -> None:
        self._store = store
        ...
        # Retention policy parameters (Gap-fix C M-8). The default cadence
        # is hourly. The age cutoff stays None so the store's configured
        # TTL (TRITIUM_DOSSIER_SIGNAL_TTL_DAYS, default 30 days) applies —
        # an explicit value here would silently override the operator's
        # env knob. Per-dossier signal counts are capped at 10000 entries
        # (oldest first).
        self._prune_interval = prune_interval
        self._prune_max_signal_age_s = prune_max_signal_age_s
        self._prune_max_signals_per_dossier = prune_max_signals_per_dossier
    ...
    def find_or_create_for_target(self, target_id: str, **kwargs) -> str:
        ...
        with self._lock:
            self._target_dossier_map[target_id] = dossier_id
        logger.info("Created dossier {} for target {}", dossier_id[:8], target_id)
    ...
    def merge(self, primary_id: str, secondary_id: str) -> bool:
        ...
        if result:
            ...
            logger.info("Merged dossier {} into {}", secondary_id[:8], primary_id[:8])
        return result
    ...
    def list_dossiers(
        ...
        self._attach_linking_target_ids(page)
        return page

    def _attach_linking_target_ids(self, dossiers: list[dict]) -> None:
        """Stamp each dossier with the unified target_id(s) it was built from.

        The store row knows nothing of the live tracker; the linkage lives only
        in ``_target_dossier_map`` (target_id -> dossier_id).  Exposing its
        reverse lets a caller navigate dossier -> live tracked target (the
        dossier->map direction of the Investigate loop) — without it the dossier
        list is orphaned from the tactical map.  A merged dossier may have
        several source target_ids: expose one representative as ``target_id``
        plus the full set as ``source_target_ids``.
        """
        with self._lock:
            reverse: dict[str, list[str]] = {}
            for tid, did in self._target_dossier_map.items():
                reverse.setdefault(did, []).append(tid)
        for d in dossiers:
            tids = reverse.get(d.get("dossier_id"))
            if not tids or d.get("target_id"):
                continue
            d["target_id"] = tids[0]
            if len(tids) > 1:
                d["source_target_ids"] = sorted(tids)
    ...
    def _handle_ble_presence(self, data: dict) -> None:
        ...
        for dev in devices:
            ...
            conf = max(0.0, min(1.0, (dev.get("rssi", -100) + 100) / 70))
            ...
            # Fuse the RSSI-derived confidence + recognised device type into the
            # dossier's typed fields so an operator sees the real class instead
            # of the "unknown / 0.0" placeholder it was born with.
            self._store.update_classification(
                dossier_id,
                entity_type=_specific_entity_type(entity_type),
                confidence=conf,
            )
    ...
    def _handle_ble_sighting(self, data: dict) -> None:
        ...
        conf = max(0.0, min(1.0, (sighting.get("rssi", -100) + 100) / 70))
        self.add_signal_to_target(
            target_id,
            source="ble",
        ...
        # Raise dossier confidence; "device" is generic so it never clobbers a
        # specific prior type (see _specific_entity_type).
        self._store.update_classification(
            dossier_id,
            entity_type=_specific_entity_type("device"),
            confidence=conf,
        )
    ...
    def _handle_ble_device(self, data: dict) -> None:
        ...
        conf = max(0.0, min(1.0, (data.get("rssi", -100) + 100) / 70))
        self.add_signal_to_target(
            target_id,
            source="ble",
    ...
    def _handle_wifi_presence(self, data: dict) -> None:
        ...
        if enriched_count:
            logger.info(
                "WiFi probe enriched {} BLE dossiers from node {} ({} SSIDs)",
    ...
    def _handle_meshtastic(self, data: dict) -> None:
        ...
        for node in nodes:
            ...
            # A mesh radio is a confident, specific identification — fuse it so
            # the dossier reads "mesh_radio / 0.9", not "unknown / 0.0".
            self._store.update_classification(
                dossier_id,
                entity_type=_specific_entity_type("mesh_radio"),
                confidence=0.9,
            )
    ...
    def _handle_detection(self, data: dict) -> None:
        ...
        for det in detections:
            ...
            entity_type = entity_type_for_class(class_name)
            ...
            # Fuse this classification into the dossier's typed fields so an
            # operator investigating the target sees the real class +
            # confidence, not the "unknown / 0.0" placeholder the dossier was
            # born with.  A bare/unrecognised class still raises confidence
            # but must not clobber a real prior entity_type with "unknown".
            self._store.update_classification(
                dossier_id,
    ...
    def _sync_tracker_targets(self) -> None:
        ...
        for target in all_targets:
            ...
            # Determine entity type from target source/asset_type.  Computed
            # for every target (not just new ones) so the classification fuse
            # below can pass a current entity_type even for existing dossiers.
            entity_type = "unknown"
            ...
            if tid.startswith("ble_"):
                ...
                # Resolve via the shared taxonomy so vision contacts get the
                # full people/vehicle/animal/phone mapping (a detected dog or
                # cell phone classifies correctly instead of falling through to
                # "unknown"), matching the _handle_detection event path.
                cls = target.classification or target.asset_type or ""
                entity_type = entity_type_for_class(cls)
            ...
            # Fuse the tracker's live classification confidence into the
            # dossier's typed fields so the Investigate-Target headline tracks
            # the classifier instead of the create-time 0.0 placeholder.  This
            # runs every sync pass (not just on creation) and is monotonic, so
            # a stronger sighting raises confidence + upgrades the type while a
            # weaker one never downgrades.  Skip when the tracker has asserted
            # no confidence yet (0.0) — honest: an unscored target stays 0.0
            # rather than inheriting a fabricated number.
            class_conf = float(
                getattr(target, "classification_confidence", 0.0) or 0.0)
            if class_conf > 0.0:
                self._store.update_classification(
                    dossier_id,
                    entity_type=_specific_entity_type(entity_type),
        ...
        if created > 0:
            logger.info("Tracker sync: created {} new dossiers ({} total targets)",
                        created, len(all_targets))

    def _flush_loop(self) -> None:
        """Periodically flush dirty dossiers, sync tracker targets, prune."""
        while self._running:
    ...
    def _maybe_prune(self) -> None:
        """Run the retention prune job at most every ``_prune_interval`` s.

        Drops ``dossier_signals`` rows older than the age cutoff for
        non-exempt dossiers (exempt = pinned OR 'vip' tag), and caps each
        dossier's signal history at ``_prune_max_signals_per_dossier``.
        The age cutoff is ``_prune_max_signal_age_s`` when set, else the
        store's configured TTL (env-tunable).  Closes Gap-fix C M-8 — the
        observed 5.3 GB / 50 GB-90d unbounded growth.
        """
        if self._prune_interval <= 0.0:
            return
        now = time.time()
        ...
        prune_fn = getattr(self._store, "prune", None)
        if prune_fn is None:
            return  # in-memory or legacy store without retention support
        try:
            stats = prune_fn(
                max_signal_age_s=self._prune_max_signal_age_s,
                max_signals_per_dossier=self._prune_max_signals_per_dossier,
                now=now,
            )
            if stats and (stats.get("aged_out") or stats.get("capped_out")):
                max_age = (
                    "%.0fs" % self._prune_max_signal_age_s
                    if self._prune_max_signal_age_s is not None
                    else "store TTL"
                )
                logger.info(
                    "Dossier retention prune: aged_out={} capped_out={} "
                    "(max_age={} cap={})",
                    stats.get("aged_out", 0), stats.get("capped_out", 0),
                    max_age,
                    self._prune_max_signals_per_dossier,
                )
        except Exception:
            logger.warning("Dossier retention prune failed", exc_info=True)
    ...
    def _flush_dirty(self) -> None:
        ...
        logger.debug("Flushed {} dirty dossiers", len(dirty_ids))
