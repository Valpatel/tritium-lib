...
Uses ProximityRule and ProximityAlert from tritium-lib models.
"""
...
# Import from tritium-lib models
try:
...
class ProximityMonitor:
    ...
    ----------
    target_tracker:
        Object with get_all() returning dict of targets with
        target_id, alliance, position attributes.
    event_bus:
        Object with publish(topic, data=...) for proximity alerts.
    scan_interval_s:
        How often to scan for proximity violations (default 2.0s).
    data_dir:
        Directory for persisting rules. Default uses DATA_DIR env var.
    """
    ...
    def __init__(
        ...
        if data_dir is not None:
            self._data_dir = Path(data_dir) / "proximity"
        else:
            self._data_dir = Path(os.environ.get("DATA_DIR", "data")) / "proximity"
    ...
    def _load_rules(self) -> None:
        ...
        rules_file = self._data_dir / "rules.json"
