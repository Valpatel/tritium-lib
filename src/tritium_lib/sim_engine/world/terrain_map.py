...
Divides the battlespace into a grid of cells (default 5m resolution).
...
Migrated from tritium-sc/src/engine/simulation/terrain.py during
sim engine unification (Phase 4: World).  Renamed to terrain_map.py to
avoid collision with the lib's existing terrain.py (heightmap/noise module).
"""
...
# Type for optional unit-type checker callback
UnitTypeChecker = Callable[[str], bool]
...
class TerrainMap:
    ...
    Args:
        map_bounds: Half-extent of the map in meters.
        resolution: Cell size in meters (default 5.0).
        is_flying_checker: Optional callback ``(asset_type: str) -> bool``
            to check if a unit type is flying.  If not provided, only the
            hardcoded ``_FLYING_TYPES`` set is used.
    """
    ...
    def __init__(
        ...
        is_flying_checker: UnitTypeChecker | None = None,
    ) -> None:
    ...
    def get_speed_modifier(self, x: float, y: float, asset_type: str) -> float:
        ...
        # Also check via callback for flying types defined in the registry
        if self._is_flying_checker is not None:
            try:
                if self._is_flying_checker(asset_type):
                    return 1.0
            except Exception:
                pass
...
def _point_in_polygon(px: float, py: float, polygon: list) -> bool:
    ...
    Uses the local implementation to avoid import dependency on tritium_lib.geo.
    """
    n = len(polygon)
    inside = False
    j = n - 1
