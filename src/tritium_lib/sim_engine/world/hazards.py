...
The event_bus parameter accepts any object with a ``publish(topic, data)``
method.  In SC this is the EventBus; in standalone simulations it can be
any compatible pub/sub bus or None (hazards still work, just no events).

Migrated from tritium-sc/src/engine/simulation/hazards.py during
sim engine unification (Phase 4: World).
"""
