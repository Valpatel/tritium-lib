...
class TestDossierManagerCRUD:
    ...
    @pytest.mark.unit
    def test_list_dossiers_carries_linking_target_id(self):
        """Loop 6 dossier->map direction: each listed dossier must expose the
        unified target_id it was built from, so the Dossier panel can deep-link
        a row back to its live map target.  Reverse of _target_dossier_map."""
        store = _make_store()
        mgr = DossierManager(store=store)
        did = mgr.find_or_create_for_target("hostile_42", name="Bandit")

        listed = mgr.list_dossiers(limit=50)
        match = [d for d in listed if d.get("dossier_id") == did]
        assert match, "created dossier not returned by list_dossiers"
        assert match[0].get("target_id") == "hostile_42", (
            "dossier list row is orphaned from its tracked target — "
            f"target_id missing/wrong: {match[0].get('target_id')!r}"
        )
        store.close()

    @pytest.mark.unit
    def test_list_dossiers_exposes_all_source_target_ids_after_merge(self):
        """When several target_ids correlate to one dossier, the list exposes
        a representative target_id plus the full source set."""
        store = _make_store()
        mgr = DossierManager(store=store)
        did = mgr.find_or_create_for_target("hostile_7", name="A")
        # Point a second target_id at the same dossier (post-correlation merge).
        with mgr._lock:
            mgr._target_dossier_map["ble_aabbccddeeff"] = did

        listed = mgr.list_dossiers(limit=50)
        match = [d for d in listed if d.get("dossier_id") == did]
        assert match
        assert match[0].get("target_id") in {"hostile_7", "ble_aabbccddeeff"}
        assert set(match[0].get("source_target_ids", [])) == {
            "hostile_7", "ble_aabbccddeeff",
        }
        store.close()
...
class TestEventHandling:
    ...
    @pytest.mark.unit
    def test_entity_type_for_class_taxonomy(self):
        """The class->entity_type map covers the four mission target kinds,
        including the generic labels the synthetic detector emits."""
        from tritium_lib.tracking.dossier_manager import entity_type_for_class
        assert entity_type_for_class("person") == "person"
        assert entity_type_for_class("car") == "vehicle"
        assert entity_type_for_class("vehicle") == "vehicle"  # generic label
        assert entity_type_for_class("dog") == "animal"
        assert entity_type_for_class("animal") == "animal"    # generic label
        assert entity_type_for_class("Dog") == "animal"       # case-insensitive
        assert entity_type_for_class("backpack") == "unknown"
        assert entity_type_for_class("") == "unknown"

    @pytest.mark.unit
    def test_handle_detection_animal_entity_type(self):
        """Animals are a first-class mission target — a dog must classify as
        'animal', not silently fall through to 'unknown'."""
        store = _make_store()
        mgr = DossierManager(store=store)
        mgr._handle_detection({
            "detections": [
                {"class_name": "dog", "confidence": 0.77, "target_id": "det_dog_1"},
            ],
        })
        d = mgr.get_dossier_for_target("det_dog_1")
        assert d is not None
        assert d["entity_type"] == "animal"
        store.close()

    @pytest.mark.unit
    def test_handle_detection_propagates_confidence_to_dossier(self):
        """The detection's confidence must reach the dossier's typed field so
        an operator investigating the target sees the real class confidence,
        not the 'unknown / 0.0' placeholder it was created with."""
        store = _make_store()
        mgr = DossierManager(store=store)
        mgr._handle_detection({
            "detections": [
                {"class_name": "dog", "confidence": 0.77, "target_id": "det_dog_2"},
            ],
        })
        d = mgr.get_dossier_for_target("det_dog_2")
        assert d is not None
    ...
    @pytest.mark.unit
    def test_handle_detection_confidence_is_monotonic(self):
        """A weaker later detection never downgrades the dossier confidence."""
        store = _make_store()
        mgr = DossierManager(store=store)
        for conf in (0.9, 0.5):
            mgr._handle_detection({
                "detections": [
                    {"class_name": "car", "confidence": conf, "target_id": "det_car_1"},
                ],
            })
        d = mgr.get_dossier_for_target("det_car_1")
        assert d is not None
    ...
    @pytest.mark.unit
    def test_handle_detection_unknown_class_does_not_clobber_type(self):
        """An unknown-class detection bumps confidence but must not overwrite
        a real prior classification with 'unknown'."""
        store = _make_store()
        mgr = DossierManager(store=store)
        # First a strong animal id, then a weaker generic 'detection' label.
        mgr._handle_detection({"detections": [
            {"class_name": "cat", "confidence": 0.6, "target_id": "det_x"}]})
        mgr._handle_detection({"detections": [
            {"class_name": "thingamajig", "confidence": 0.95, "target_id": "det_x"}]})
        d = mgr.get_dossier_for_target("det_x")
        assert d is not None
        assert d["entity_type"] == "animal"  # not downgraded to unknown
        assert d["confidence"] == pytest.approx(0.95)  # confidence still climbs
        store.close()

    @pytest.mark.unit
    def test_entity_type_for_class_phone(self):
        """Phones are the 4th mission target ('cars, phones, people, animals').
        A YOLO 'cell phone' detection must classify as 'phone', matching the
        entity_type BLE presence already uses for phones, so RF+vision fuse
        onto one taxonomy instead of vision phones falling through to unknown."""
        from tritium_lib.tracking.dossier_manager import entity_type_for_class
        assert entity_type_for_class("cell phone") == "phone"
        assert entity_type_for_class("cellphone") == "phone"
        assert entity_type_for_class("smartphone") == "phone"
        assert entity_type_for_class("mobile phone") == "phone"
        assert entity_type_for_class("Cell Phone") == "phone"  # case-insensitive

    @pytest.mark.unit
    def test_handle_detection_cell_phone_entity_type(self):
        """A vision 'cell phone' detection resolves to the phone target type."""
        store = _make_store()
        mgr = DossierManager(store=store)
        mgr._handle_detection({"detections": [
            {"class_name": "cell phone", "confidence": 0.81,
             "target_id": "det_phone_1"}]})
        d = mgr.get_dossier_for_target("det_phone_1")
        assert d is not None
        assert d["entity_type"] == "phone"
        assert d["confidence"] == pytest.approx(0.81)
        store.close()

    @pytest.mark.unit
    def test_handle_ble_presence_propagates_confidence(self):
        """BLE presence must fuse its RSSI-derived confidence + recognised
        device type into the dossier's typed fields, not leave them frozen at
        the 'unknown / 0.0' the dossier was born with (the same cross-sensor
        classification fusion the vision path already does)."""
        store = _make_store()
        mgr = DossierManager(store=store)
        mgr._handle_ble_presence({"devices": [
            {"mac": "AA:BB:CC:DD:EE:01", "name": "Pixel", "rssi": -40,
             "type": "phone"}]})
        d = mgr.get_dossier_for_target("ble_aabbccddee01")
        assert d is not None
        assert d["entity_type"] == "phone"
        # rssi -40 -> (−40+100)/70 ≈ 0.857
        assert d["confidence"] == pytest.approx((-40 + 100) / 70)
        store.close()

    @pytest.mark.unit
    def test_handle_ble_sighting_raises_confidence_keeps_generic_type(self):
        """A generic BLE sighting raises dossier confidence but leaves the
        entity_type as the generic 'device' (no specific class to assert)."""
        store = _make_store()
        mgr = DossierManager(store=store)
        mgr._handle_ble_sighting({"sighting": {
            "mac": "AA:BB:CC:DD:EE:02", "rssi": -30}})
        d = mgr.get_dossier_for_target("ble_aabbccddee02")
        assert d is not None
        assert d["entity_type"] == "device"
        assert d["confidence"] == pytest.approx((-30 + 100) / 70)
        store.close()

    @pytest.mark.unit
    def test_handle_ble_generic_does_not_clobber_phone(self):
        """A strong BLE 'phone' id must not be downgraded to the generic
        'device' by a later generic sighting at higher RSSI — specificity is
        preserved while confidence still climbs."""
        store = _make_store()
        mgr = DossierManager(store=store)
        # Born as phone at moderate RSSI...
        mgr._handle_ble_presence({"devices": [
            {"mac": "AA:BB:CC:DD:EE:03", "name": "iPhone", "rssi": -40,
             "type": "phone"}]})
        # ...then a generic device sighting at stronger RSSI (higher conf).
        mgr._handle_ble_device({"mac": "AA:BB:CC:DD:EE:03", "rssi": -10})
        d = mgr.get_dossier_for_target("ble_aabbccddee03")
        assert d is not None
        assert d["entity_type"] == "phone"  # not downgraded to 'device'
        assert d["confidence"] == pytest.approx(1.0)  # rssi -10 -> capped 1.0
        store.close()

    @pytest.mark.unit
    def test_handle_meshtastic_propagates_confidence(self):
        """A meshtastic node telemetry event fuses its classification into the
        dossier so a mesh radio reads 'mesh_radio / 0.9', not 'unknown / 0.0'."""
        store = _make_store()
        mgr = DossierManager(store=store)
        mgr._handle_meshtastic({"nodes": [
            {"id": "node42", "long_name": "Relay-42", "snr": 8.5}]})
        d = mgr.get_dossier_for_target("mesh_node42")
        assert d is not None
        assert d["entity_type"] == "mesh_radio"
        assert d["confidence"] == pytest.approx(0.9)
        store.close()

    @pytest.mark.unit
    def test_sync_tracker_propagates_classification_confidence(self):
        """The catch-all tracker sync (the dominant live dossier-creation path)
        must fuse the tracker's classification_confidence into the dossier, not
        leave it frozen at the create-time 0.0 — otherwise the Investigate
        Target headline reads 'Unknown / 0%' for a confidently-typed contact."""
        tracker = _make_tracker_with(
            TrackedTarget(target_id="det_dog_9", name="Dog", alliance="neutral",
                          asset_type="animal", classification="dog",
                          classification_confidence=0.88),
        )
        store = _make_store()
        mgr = DossierManager(store=store, tracker=tracker)
        mgr._sync_tracker_targets()
        d = mgr.get_dossier_for_target("det_dog_9")
        assert d is not None
        assert d["entity_type"] == "animal"
    ...
    @pytest.mark.unit
    def test_sync_tracker_unscored_stays_zero(self):
        """A target the classifier has not scored (classification_confidence
        0.0) must stay 0.0 — the sync never fabricates a confidence."""
        tracker = _make_tracker_with(
            TrackedTarget(target_id="sdr_doorbell_1", name="Doorbell",
                          alliance="neutral", asset_type="doorbell",
        ...
        mgr = DossierManager(store=store, tracker=tracker)
        mgr._sync_tracker_targets()
        d = mgr.get_dossier_for_target("sdr_doorbell_1")
        assert d is not None
        assert d["confidence"] == pytest.approx(0.0)
        store.close()

    @pytest.mark.unit
    def test_sync_tracker_confidence_updates_on_repeat(self):
        """Re-syncing an already-created dossier raises its confidence as the
        tracker refines the classification (monotonic, no downgrade)."""
        tgt = TrackedTarget(target_id="det_car_9", name="Car", alliance="neutral",
                            asset_type="vehicle", classification="car",
                            classification_confidence=0.5)
        tracker = _make_tracker_with(tgt)
        store = _make_store()
        mgr = DossierManager(store=store, tracker=tracker)
        mgr._sync_tracker_targets()  # creates at 0.5
        tgt.classification_confidence = 0.9
        mgr._sync_tracker_targets()  # raises to 0.9
        d = mgr.get_dossier_for_target("det_car_9")
        assert d["confidence"] == pytest.approx(0.9)
        tgt.classification_confidence = 0.3
        mgr._sync_tracker_targets()  # weaker, must NOT downgrade
        d = mgr.get_dossier_for_target("det_car_9")
        assert d["confidence"] == pytest.approx(0.9)
        assert d["entity_type"] == "vehicle"
        store.close()
...
class TestRetentionPruneWiring:
    """SC -> lib retention wiring: the operator TTL knob must flow through.

    Regression guard for the 2026-06-11 finding: DossierManager passed a
    hardcoded 30-day ``max_signal_age_s`` to ``store.prune()``, and per
    the lib contract an explicit value always wins over the store's
    configured TTL — which made ``TRITIUM_DOSSIER_SIGNAL_TTL_DAYS`` a
    dead knob in every SC deployment.
    """

    def _force_prune(self, mgr):
        mgr._last_prune = 0.0
        mgr._maybe_prune()

    @pytest.mark.unit
    def test_env_ttl_flows_through_manager_prune(self, monkeypatch):
        monkeypatch.setenv("TRITIUM_DOSSIER_SIGNAL_TTL_DAYS", "1")
        store = _make_store()  # TTL resolved at construction: 1 day
        try:
            mgr = DossierManager(store=store)  # no explicit age override
            did = mgr.find_or_create_for_target("t1")
            store.add_signal(did, "ble", "sighting",
                             timestamp=time.time() - 2 * 86400)
            self._force_prune(mgr)
            signals = mgr.get_dossier_for_target("t1")["signals"]
            assert len(signals) == 0, (
                "2-day-old signal survived a 1-day env TTL — the manager "
                "is overriding TRITIUM_DOSSIER_SIGNAL_TTL_DAYS"
            )
        finally:
            store.close()

    @pytest.mark.unit
    def test_explicit_age_param_wins_over_env(self, monkeypatch):
        monkeypatch.setenv("TRITIUM_DOSSIER_SIGNAL_TTL_DAYS", "1")
        store = _make_store()
        try:
            mgr = DossierManager(store=store,
                                 prune_max_signal_age_s=10 * 86400.0)
            did = mgr.find_or_create_for_target("t1")
            store.add_signal(did, "ble", "sighting",
                             timestamp=time.time() - 2 * 86400)
            self._force_prune(mgr)
            ...
            assert len(signals) == 1, (
                "explicit prune_max_signal_age_s should win over the env TTL"
            )
        finally:
            store.close()

    @pytest.mark.unit
    def test_zero_interval_disables_prune(self):
        store = _make_store()
        try:
            mgr = DossierManager(store=store, prune_interval=0.0)
            did = mgr.find_or_create_for_target("t1")
            store.add_signal(did, "ble", "sighting",
                             timestamp=time.time() - 400 * 86400)
            self._force_prune(mgr)
            signals = mgr.get_dossier_for_target("t1")["signals"]
            assert len(signals) == 1, "prune_interval=0 must disable pruning"
        finally:
            store.close()
